...
bandits="/home/scripts/bandits/das_bandits.R"
...
if [[ "$salmonstar" = "y" ]]; then
	...
	echo "$(($(date +%s)-$starter))" >> $log/bandits-salmon-star_${dir}.$(date +%s).runtime
	kill -15 $wid
fi
...
if [[ "$kallisto" = "y" ]]; then
	...
	echo "$(($(date +%s)-$starter))" >> $log/bandits-kallisto_${dir}.$(date +%s).runtime
	kill -15 $wid
fi
